...
export class ForecastContainer extends Component {
  ...
  render() {
    ...
    const isTodayForecastLoaded =
      todayForecast && Object.entries(todayForecast).length;
    const isFiveDayForecastLoaded = fiveDayForecast && fiveDayForecast.length;
    if (fetching || !isTodayForecastLoaded || !isFiveDayForecastLoaded)
      return <div>Loading...</div>;
    const todayData = {
      cityCard: {
        city: todayForecast.name,
        description:
          todayForecast.weather.length && todayForecast.weather[0].description,
