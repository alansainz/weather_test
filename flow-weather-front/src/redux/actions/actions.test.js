...
describe("get today's forecast OK", () => {
  it("handles getting today's forecast and fetches all necessary data", async () => {
    ...
    expect(await getAction(store, GET_TODAY_FORECAST_ATTEMPT)).toEqual(
      resultAttempt
    );
    expect(await getAction(store, GET_TODAY_FORECAST_SUCCESS)).toMatchObject(
      resultSuccess
    );
  });
});
...
describe("get today's forecast WRONG", () => {
  it("handles getting an error during the today's forecast request", async () => {
    ...
    expect(await getAction(store, GET_TODAY_FORECAST_FAILURE)).toMatchObject(
      resultFailure
    );
  });
});
...
describe('get five days forecast OK', () => {
  it('handles getting five days forecast and fetches all necessary data', async () => {
    ...
    expect(await getAction(store, GET_FIVE_DAYS_FORECAST_ATTEMPT)).toEqual(
      resultAttempt
    );
    expect(
      await getAction(store, GET_FIVE_DAYS_FORECAST_SUCCESS)
    ).toMatchObject(resultSuccess);
  });
});
...
describe('get five days forecast WRONG', () => {
  it('handles getting an error during the five days request', async () => {
    ...
    expect(
      await getAction(store, GET_FIVE_DAYS_FORECAST_FAILURE)
    ).toMatchObject(resultFailure);
  });
});
